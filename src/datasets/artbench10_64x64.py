...
class ArtBench10_64x64:
    def __init__(self, root, train: bool = True, limit: int = None):
        ...
        self.resize = torchvision.transforms.Resize(size=(64, 64))
    ...
    def __getitem__(self, index):
        return (
            TF.normalize(
                self.resize(torchvision.io.read_image(self.img_path[index]).to(torch.float32) / 255.0),
                (0.5, 0.5, 0.5),
                (0.5, 0.5, 0.5),
            ),
            self.target[index],
